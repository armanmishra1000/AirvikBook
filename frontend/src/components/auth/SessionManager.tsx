...
export const SessionManager: React.FC<SessionManagerProps> = ({
  ...
  const loadSessions = async () => {
    ...
    try {
      ...
      if (isSuccessResponse(result) && mountedRef.current) {
        ...
        if (result.code === 'NETWORK_ERROR' || result.code === 'MISSING_TOKEN') {
      ...
      if (mountedRef.current) {
        setError('Network error while loading sessions. Please check your connection.');
        setRetryCount(prev => prev + 1);
      }
    } finally {
      setIsLoading(false);
  ...
  }, [authState.isAuthenticated]); // Only depend on authentication status, not user object
    ...
    return (
      <div className={`w-full ${className}`}>
        <div className="animate-pulse space-y-space-4">
          <div className="w-1/3 h-6 bg-gray-200 dark:bg-gray-700 rounded-radius-md" />
  ...
  return (
    <div className={`w-full ${className}`}>
      ...
      {/* Error Message */}
      {error && (
        <div className="border border-red-200 mb-space-6 bg-red-50 dark:bg-red-900/20 dark:border-red-800 rounded-radius-md p-space-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              <svg className="w-5 h-5 text-error mr-space-3" viewBox="0 0 20 20" fill="currentColor">
            ...
            {retryCount < 3 && (
              <button
                ...
                className="text-sm underline text-error hover:text-red-700"
              >
                Retry
              </button>
            )}
      ...
      {sessions.length === 0 ? (
        <div className="text-center py-space-12">
          <div className="w-12 h-12 mx-auto text-gray-400 dark:text-gray-600 mb-space-4">
            <svg fill="currentColor" viewBox="0 0 20 20">
              <path fillRule="evenodd" d="M3 4a1 1 0 011-1h12a1 1 0 011 1v8a1 1 0 01-1 1h-4v1h2a1 1 0 110 2H6a1 1 0 110-2h2v-1H4a1 1 0 01-1-1V4zm1 1v6h12V5H4z" clipRule="evenodd" />
            </svg>
          </div>
          <h3 className="text-gray-900 text-h5 font-sf-pro dark:text-gray-100 mb-space-2">
            No Active Sessions
          </h3>
          <p className="text-gray-500 text-body dark:text-gray-400">
            {error ? 'Unable to load sessions. Please try again later.' : 'You don\'t have any active sessions at the moment.'}
          </p>
        </div>
      ) : (
            ...
            >
              ...
              {session.isCurrent && (
                <div className="absolute top-space-2 right-space-2">
                  <span className="inline-flex items-center font-medium px-space-2 py-space-1 rounded-radius-full text-caption font-sf-pro bg-airvik-blue text-airvik-white">
                    Current Session
                  </span>
                </div>
              )}
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-start flex-1 space-x-space-4">
                  {/* Device Icon */}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center space-x-space-2 mb-space-1">
                      <h4 className="truncate text-h6 font-sf-pro text-airvik-black dark:text-airvik-white">
                        {session.deviceInfo.deviceName}
                      </h4>
                    </div>
                    
                    <div className="text-gray-600 space-y-space-1 text-body dark:text-gray-400">
                ...
                {!session.isCurrent && (
                  <div className="flex-shrink-0 ml-space-4">
                    <button
                      ...
                      className="text-gray-700 transition-colors border border-gray-300 px-space-3 py-space-2 text-caption dark:border-gray-600 rounded-radius-md dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-airvik-blue focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed duration-normal font-sf-pro"
                    >
                      {isLogoutLoading === session.id ? (
                        <div className="flex items-center">
                          <div className="w-3 h-3 border border-gray-400 animate-spin mr-space-1 border-t-transparent rounded-radius-full" />
                          Logging out...
                        </div>
                      ) : (
                        'Logout'
      ...
      {/* Security Notice */}
      <div className="border border-blue-200 mt-space-8 p-space-4 bg-blue-50 dark:bg-blue-900/20 dark:border-blue-800 rounded-radius-md">
        <div className="flex items-start">
          <div className="flex-shrink-0">
            <svg className="w-5 h-5 text-airvik-blue mt-space-1" viewBox="0 0 20 20" fill="currentColor">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-space-4">
          <div className="w-full max-w-md bg-airvik-white dark:bg-gray-800 rounded-radius-lg p-space-6">
            <div className="text-center">
              <div className="flex items-center justify-center w-12 h-12 mx-auto bg-red-100 rounded-radius-full dark:bg-red-900/20 mb-space-4">
              ...
              <div className="flex space-x-space-4">
                ...
                <button
                  ...
                  className="flex-1 transition-colors px-space-4 py-space-2 bg-error text-airvik-white rounded-radius-md hover:bg-red-700 duration-normal font-sf-pro text-button"
                >
                  Logout All
                </button>
              </div>
