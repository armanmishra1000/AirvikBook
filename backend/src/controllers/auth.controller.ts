...
export class AuthController {
  ...
  static async refreshToken(req: Request, res: Response): Promise<Response | void> {
    try {
      ...
      // Use token rotation (generates new access token AND new refresh token)
      const rotationResult = await JwtService.rotateTokens(refreshToken);

      if (!rotationResult.success) {
        const statusCode = rotationResult.code === 'TOKEN_REVOKED' || 
                          rotationResult.code === 'REFRESH_TOKEN_EXPIRED' ? 401 : 400;
        return ResponseUtil.error(
          res,
   ...
   */
  static async logoutAll(req: Request, res: Response): Promise<Response | void> {
    try {
      ...
      const success = await JwtService.logoutUser(userId);

      if (!success) {
        return ResponseUtil.error(res, 'Failed to logout from all devices', 'LOGOUT_FAILED', 500);
      }
